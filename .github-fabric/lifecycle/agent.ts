/**
 * agent.ts — Core agent orchestrator for GitHub Fabric.
 *
 * ─────────────────────────────────────────────────────────────────────────────
 * PURPOSE
 * ─────────────────────────────────────────────────────────────────────────────
 * This is the main entry point for the GitHub Fabric AI coding agent.  It receives
 ...
 * SESSION CONTINUITY
 * ─────────────────────────────────────────────────────────────────────────────
 * GitHub Fabric maintains per-issue session state in:
 *   .github-fabric/state/issues/<number>.json   — maps issue # → session file path
 *   .github-fabric/state/sessions/<timestamp>.jsonl — the `pi` session transcript
 *
...
// `import.meta.dir` resolves to `.github-fabric/lifecycle/`; stepping up one level
// gives us the `.github-fabric/` directory which contains `state/` and `node_modules/`.
const fabricDir = resolve(import.meta.dir, "..");
const stateDir = resolve(fabricDir, "state");
const issuesDir = resolve(stateDir, "issues");
const sessionsDir = resolve(stateDir, "sessions");
const piSettingsPath = resolve(fabricDir, ".pi", "settings.json");
...
const sessionsDirRelative = ".github-fabric/state/sessions";
...
try {
  ...
  await run(["git", "config", "user.name", "github-fabric[bot]"]);
  await run(["git", "config", "user.email", "github-fabric[bot]@users.noreply.github.com"]);
  ...
  const piBin = resolve(fabricDir, "node_modules", ".bin", "pi");
  const piArgs = [
    piBin,
    "--mode",
    "json",
  ...
  if (exitCode !== 0) {
    ...
    const commitResult = await run(["git", "commit", "-m", `fabric: work on issue #${issueNumber}`]);
