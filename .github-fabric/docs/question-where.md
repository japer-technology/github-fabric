...
If *[The Repo Is the Mind](the-repo-is-the-mind.md)* is the thesis, then this repository is its field evidence. The “where” is not metaphorical. GitHub Fabric answers it with a physical address: inside the repository’s own boundaries, executed by the same automation that already builds and ships code.
...
The workflow template (`.github-fabric/install/github-fabric-agent.yml`) is explicit: issue events and issue comments trigger a job, collaborator permissions are checked, the repository is checked out, Bun is installed, and the agent scripts run locally in the Actions runner.
...
The orchestrator (`.github-fabric/lifecycle/agent.ts`) persists issue-to-session mappings and session transcripts under `.github-fabric/state/`. The README repeats the same mapping model: issue number → state file → session transcript. This is not decorative documentation; it is the mechanism of continuity.

So “where memory lives” is no longer a hand-wavy concept about context windows. It lives in:

- `.github-fabric/state/issues/<number>.json`
- `.github-fabric/state/sessions/<session>.jsonl`
...
The installer script (`FABRIC-INSTALLER.ts`) shows the project’s ergonomics: copy workflow template, copy hatch template, initialize `AGENTS.md`, install dependencies. The root `setup.sh` mirrors that one-command posture.

This is subtle but essential. By making installation file-centric, the project ensures the answer to “where is the product?” remains simple: in `.github-fabric/` plus generated `.github/` workflow/template files. The “platform” is a commit.
...
GitHub Fabric’s deepest claim is that intelligence should not orbit the repository as an external service. It should inhabit the repository as a first-class, auditable participant. This codebase demonstrates that claim concretely: the “where” of AI is not a dashboard, not a chat tab, not a vendor silo.
