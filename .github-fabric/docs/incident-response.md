...
This document provides precise, step-by-step procedures for **taking control** of a security incident involving the `github-fabric` (GMI) agent. It covers containment, manual file deletion of compromised artifacts, disabling the workflow agent, and stopping any existing GitHub Actions runs.
...
gh workflow disable "github-fabric-agent.yml"
```

> **Verification:** Confirm under **Actions → github-fabric-agent** that the badge reads "This workflow is disabled" and no runs are in progress.
...
find .github-fabric -type f -newer security-assessment.md -ls
```
...
git rm -r .github-fabric/state/sessions/*
git rm -r .github-fabric/state/issues/*
...
diff .github/workflows/github-fabric-agent.yml \
     .github-fabric/install/github-fabric-agent.yml
```
...
git diff HEAD~20 -- .github-fabric/lifecycle/
git diff HEAD~20 -- .github-fabric/.pi/
```
...
gh workflow enable "github-fabric-agent.yml"
```
