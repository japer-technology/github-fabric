...
GitHub Fabric clarifies that execution authority is infrastructural, not mystical.
...
GitHub Fabric is strongest where it is least romantic: it maps power to explicit config, and config to repository ownership. This is why the model can be swappable, the package can be pinned, and behavior can remain under local control.
...
In GitHub Fabric, identity is not hidden in a proprietary backend. It is staged, negotiated, and persisted in the same medium as software itself. The repository does not merely *contain* intelligence. It defines who that intelligence is allowed to become.
